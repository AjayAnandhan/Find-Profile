...
if os.path.exists("resume_index.faiss"):
    index = faiss.read_index("resume_index.faiss")

else:
    index = None

if os.path.exists("resume_metadata.pkl"):
...
def search_candidates(jd, top_k=100):
    ...
    for rank, idx in enumerate(indices[0]):
        ...
        semantic_score = round((1 / (1 + distances[0][rank])) * 100, 2)

        skill_match = skill_score(jd, row[4])

        matched_skills, missing_skills = skill_analysis(jd, row[4])

        exp_match = experience_score(jd_exp, row[3])

        final_score = round(
            semantic_score * 0.70 + skill_match * 0.20 + exp_match * 0.10, 2
        )
